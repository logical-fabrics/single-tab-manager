...
/**
 * URLが設定されたパターンのいずれかに一致するかチェック
 * @param {string} url - チェックするURL
 * @param {RegExp[]} patterns - 正規表現パターンの配列
 * @returns {boolean} パターンに一致する場合true
 */
 ...
 */
async function findMatchingTabs(targetUrl, excludeTabId, patterns) {
  try {
    ...
    for (const tab of allTabs) {
      ...
      // URLが同じパターンに一致するかチェック
      if (tab.url && matchesAnyPattern(tab.url, patterns)) {
 ...
 */
async function closeOldTabsAndActivateNew(newTabId, oldTabs) {
  try {
    // 古いタブを閉じる
    const oldTabIds = oldTabs.map(tab => tab.id);
    if (oldTabIds.length > 0) {
      await chrome.tabs.remove(oldTabIds);
      console.log(LOG_PREFIX, `Closed ${oldTabIds.length} old tab(s):`, oldTabIds);
    }
...
async function handleTabUrl(tabId, url) {
  ...
  // URLがパターンに一致するかチェック
  if (!matchesAnyPattern(url, patterns)) {
    console.log(LOG_PREFIX, `URL does not match any pattern: ${url}`);
    return;
  }

  console.log(LOG_PREFIX, `URL matches pattern: ${url}`);

  // 同じパターンに一致する既存タブを検索
  const matchingTabs = await findMatchingTabs(url, tabId, patterns);

  if (matchingTabs.length > 0) {
    console.log(LOG_PREFIX, `Found ${matchingTabs.length} existing matching tab(s)`);
